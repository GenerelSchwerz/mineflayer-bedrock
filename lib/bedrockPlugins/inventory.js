...
function inject (bot, { hideErrors }) {
  ...
  // bot._client.on('completed_using_item', (packet) => { // eating
  //   eatingTask.finish()
  //   bot.usingHeldItem = false
  // })
  // entity_event set_entity_data
  ...
  bot._client.on('completed_using_item', (packet) => { //set_cooldown
    if (bot.heldItem && bot.heldItem.type !== packet.used_item_id) return
    if (!eatingTask.done) {
      eatingTask.finish()
    }
    bot.usingHeldItem = false
  ...
  bot._client.on('player_hotbar', (packet) => { // works
    // held item change
  ...
  bot._client.on('inventory_slot', (packet) => { // UNSUPPORTED ARMOR AND OFF HAND
    // set slot
    const window = packet.window_id === 'inventory' ? bot.inventory : null// bot.currentWindow
    if(!window) return
    if (window.type.replace('minecraft:','') !== packet.window_id) return
    const newItem = packet.item // FROM NOTCH WHEN AVALIABLE
  ...
  bot._client.on('inventory_content', (packet) => {
    const window = packet.window_id === 'inventory' ? bot.inventory : null//bot.currentWindow
    if(!window) return
    if (window.type.replace('minecraft:','') !== packet.window_id) {
      windowItems = packet
      return
    }
    ...
    for (let i = 0; i < packet.input.length; ++i) {
      const item = packet.input[i]
      // 854 - expected steak, item.network_id - 274 find out how thats possible
      // new Item(item.network_id, item.count, item.metadata, item.extra?.nbt)
      // use fromNotch after bedrock support
