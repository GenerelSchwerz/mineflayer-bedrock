...
function inject (bot, options) {
  ...
  // temporary fix for chat, until bedrock-provider supports it.
  bot._client.chat = (text) => {
    bot._client.queue('text', {
        type: 'chat',
        needs_translation: false,
        source_name: bot._client.profile.name,
        message: text,
        xuid: bot._client.profile.xuid.toString(),
        platform_chat_id: '',
        filtered_message: '',
      });
}
  ...
  bot._client.on('text', (data) => {
    ...
    switch (data.type) {
      case 'chat': 
        bot.emit('chat', data.source_name, message)
      default:
        break
    ...
    //   const parameters = {
    //     sender: data.source_name ? JSON.parse(data.senderName) : undefined,
    ...
    // }
    bot.emit('message', message, 'chat', data.source_name, verified)
    bot.emit('messagestr', message, 'chat', message, data.source_name, verified)
  })
